...
- **Fixed the bride portal to show a separate timeline per stylist.** The studio-side Timeline tab has shown parallel per-stylist schedules since #10, but the public bride portal (`/portal/[bookingId]`) never got the same treatment — the `get_bride_portal` RPC didn't even return `assigned_stylist_id`, a stylist roster, or `start_time`, so the frontend had no way to know who was doing whose hair/makeup and just computed one flat timeline for the whole party. On a job with 3 stylists, the bride only ever saw one merged schedule instead of 3. Updated `supabase/schema.sql`'s `get_bride_portal` to return `assigned_stylist_id` on each party member, `start_time` on the booking, and a `stylists` array (id/name/role) from `booking_stylists`. Updated `BridePortalData` in `lib/types.ts` to match, and rewired `app/portal/[bookingId]/page.tsx` to group members by stylist and compute one timeline per group (same `computeTimeline`/`computeTimelineFromStart` logic and lead-first sort the studio side uses), rendering a labeled section per stylist (name + Lead/Assist badge) when more than one is assigned. Also fixed the buffer-minutes fallback to match the studio side (`?? 10`, was `?? 0`) so the two views agree. Needs the updated `get_bride_portal` function pushed to Supabase before this takes effect.
...
- **Re-captured calendar marketing screenshot** with current stylist assignments, no mouse cursor.
