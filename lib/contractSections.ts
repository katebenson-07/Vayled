...
/**
 * The starter contract, split into one editable card per section. Headings
 * are auto-numbered by position — no roman numerals to type or keep in sync.
 */
export const DEFAULT_SECTIONS: Omit<ContractSection, "id">[] = [
  {
    heading: "Hair Services",
    body: `The Hair stylist, {{studio_name}}, will provide hair services for the Client and bridal party members as outlined below:
...
- Bridal Party: Hair styles may be extended to the bridal party, as specified by the Client. {{studio_name}} will provide hair styling for bridesmaids, mothers, or other individuals as agreed upon.`,
  ...
  {
    heading: "Hair Stylist Responsibilities",
    body: `Consultation and Bridal Preview: {{studio_name}} will conduct an initial consultation with the Client to discuss hair preferences, style, and the overall vision for the wedding day look. If requested, a bridal preview session will be scheduled to ensure the Client's satisfaction and to finalize the hair design.
...
Touch-up Kit: {{studio_name}} and team may provide a touch-up kit, including essential hair products for minor touch-ups during the event.`,
  },
  {
    ...
    body: `Consultation and Communication: The Client will actively participate in the initial consultation with {{studio_name}}. This includes openly discussing hair preferences, style, and any specific requirements for the wedding day look. Timely communication with {{studio_name}} is crucial for a successful collaboration.
...
Payment Adherence: The Client agrees to adhere to the payment schedule outlined in the Payment section. All payments must be made as specified to secure the Hair stylist services.
...
Communication of Changes: In the event of any changes to the agreed-upon schedule or hair requirements, the Client will promptly inform the Hair stylist to accommodate adjustments as needed.`,
  ...
  {
    heading: "Payment",
    body: `In consideration of the hair services provided by the Hair Stylist for {{bride_name}}'s wedding on {{wedding_date}} at {{venue}}, the Client agrees to the following payment terms:
...
- All payments are non-refundable.`,
  },
  {
    ...
    body: `In the event of cancellation by the Client:
...
- {{studio_name}} reserves the right to cancel the Contract in case of unforeseen circumstances. In such cases, a full refund of any payments made will be issued to the client.`,
  },
  {
    ...
    body: `{{studio_name}}'s refund policy is as follows:
...
- If the Client cancels the contract, as outlined in the Cancellation section, refunds will not be provided, and any payments made are non-refundable.`,
  },
  {
    heading: "Liability",
    body: `{{studio_name}} is not responsible for any allergic reactions or adverse skin reactions that may occur as a result of hair styling. The Hair stylist will use professional, high-quality products and conduct a patch test upon request. The Client acknowledges any known allergies or sensitivities and agrees to communicate them to the Hair stylist.`,
  },
  {
    heading: "Photography Release",
    body: `The Client agrees that the hair stylist may use photographs of the completed hair style for promotional purposes, including but not limited to the artist's portfolio and social media, unless declined below.

- [ ] I do not consent to the use of my wedding photos for the Hair Stylist's promotional purposes.`,
  },
  {
    heading: "Amendments",
    body: `Any changes or amendments to this Contract must be made in writing and agreed upon by both Parties.`,
  },
  {
    heading: "Governing Law",
    body: `This Contract shall be governed by and construed in accordance with the laws of Newport, RI.`,
  },
  {
    heading: "Entire Agreement",
    body: `This contract, including all its attachments, represents the entire understanding and agreement between {{studio_name}}, hereinafter referred to as the "Hair stylist," and {{bride_name}}, hereinafter referred to as the "Client," regarding the hair services for the wedding event scheduled on {{wedding_date}}.
...
export function freshDefaultSections(): ContractSection[] {
...
/**
 * Converts an old single-blob template body (the format used before section
 * cards existed) into a sections array, so nothing anyone already wrote gets
 * lost. Any text before the first recognized heading becomes an
 * "Introduction" section rather than being dropped.
 */
export function parseLegacyBody(body: string): ContractSection[] {
  ...
  for (const block of blocks) {
    ...
    const isHeading = lines.length === 1 && HEADING_RE.test(lines[0].trim());
    if (isHeading) {
      const match = HEADING_RE.exec(lines[0].trim())!;
      sections.push({ heading: match[2], bodyLines: [] });
      continue;
    }
    if (sections.length === 0) {
      intro.push(block);
    } else {
      sections[sections.length - 1].bodyLines.push(block);
    }
  }

  const result: ContractSection[] = [];
  if (intro.length > 0) {
    result.push({ id: crypto.randomUUID(), heading: "Introduction", body: intro.join("\n\n") });
  }
  for (const s of sections) {
    result.push({ id: crypto.randomUUID(), heading: s.heading, body: s.bodyLines.join("\n\n") });
  }
  return result;
}
